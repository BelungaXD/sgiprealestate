...
Для автоматической настройки можно использовать скрипт `scripts/setup-deploy-permissions.sh`:
...
**Для быстрой настройки используйте скрипт `scripts/setup-deploy-permissions.sh`**
