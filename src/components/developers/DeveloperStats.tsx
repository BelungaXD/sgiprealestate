import { useTranslation } from 'next-i18next'
import { 
  BuildingOfficeIcon, 
  HomeIcon, 
  CurrencyDollarIcon, 
  StarIcon,
  TrophyIcon,
  GlobeAltIcon
} from '@heroicons/react/24/outline'

interface Developer {
  id: string
  name: string
  nameEn: string
  propertiesCount: number
  averagePrice: number
  currency: string
  rating: number
  marketShare: number
  countries: string[]
  awards: string[]
}

interface DeveloperStatsProps {
  developers: Developer[]
}

export default function DeveloperStats({ developers }: DeveloperStatsProps) {
  const { t } = useTranslation('developers')

  const totalProperties = developers.reduce((sum, dev) => sum + dev.propertiesCount, 0)
  const averageRating = developers.reduce((sum, dev) => sum + dev.rating, 0) / developers.length
  const totalCountries = new Set(developers.flatMap(dev => dev.countries)).size
  const totalAwards = developers.reduce((sum, dev) => sum + dev.awards.length, 0)
  const totalMarketShare = developers.reduce((sum, dev) => sum + dev.marketShare, 0)

  const formatPrice = (price: number, currency: string) => {
    return new Intl.NumberFormat('en-US', {
      style: 'currency',
      currency: currency,
      minimumFractionDigits: 0,
    }).format(price)
  }

  const stats = [
    {
      icon: BuildingOfficeIcon,
      value: developers.length,
      label: t('stats.developers'),
      description: t('stats.developersDescription'),
      color: 'text-blue-600',
      bgColor: 'bg-blue-100'
    },
    {
      icon: HomeIcon,
      value: totalProperties,
      label: t('stats.properties'),
      description: t('stats.propertiesDescription'),
      color: 'text-green-600',
      bgColor: 'bg-green-100'
    },
    {
      icon: StarIcon,
      value: averageRating.toFixed(1),
      label: t('stats.averageRating'),
      description: t('stats.averageRatingDescription'),
      color: 'text-yellow-600',
      bgColor: 'bg-yellow-100'
    },
    {
      icon: GlobeAltIcon,
      value: totalCountries,
      label: t('stats.countries'),
      description: t('stats.countriesDescription'),
      color: 'text-purple-600',
      bgColor: 'bg-purple-100'
    },
    {
      icon: TrophyIcon,
      value: totalAwards,
      label: t('stats.awards'),
      description: t('stats.awardsDescription'),
      color: 'text-orange-600',
      bgColor: 'bg-orange-100'
    },
    {
      icon: CurrencyDollarIcon,
      value: `${totalMarketShare.toFixed(1)}%`,
      label: t('stats.marketShare'),
      description: t('stats.marketShareDescription'),
      color: 'text-champagne',
      bgColor: 'bg-champagne/10'
    }
  ]

  return (
    <div>
      <div className="text-center mb-12">
        <h2 className="text-3xl font-bold text-graphite mb-4">
          {t('stats.title')}
        </h2>
        <p className="text-lg text-gray-600 max-w-3xl mx-auto">
          {t('stats.description')}
        </p>
      </div>

      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
        {stats.map((stat, index) => {
          const IconComponent = stat.icon
          return (
            <div key={index} className="text-center group hover:scale-105 transition-transform duration-300">
              <div className={`w-16 h-16 ${stat.bgColor} rounded-full flex items-center justify-center mx-auto mb-4 group-hover:scale-110 transition-transform duration-300`}>
                <IconComponent className={`h-8 w-8 ${stat.color}`} />
              </div>
              <div className="text-3xl font-bold text-graphite mb-2">
                {stat.value}
              </div>
              <div className="text-lg font-semibold text-graphite mb-2">
                {stat.label}
              </div>
              <div className="text-sm text-gray-600">
                {stat.description}
              </div>
            </div>
          )
        })}
      </div>

      {/* Top Developers by Properties */}
      <div className="mt-16">
        <h3 className="text-2xl font-bold text-graphite text-center mb-8">
          {t('topDevelopers.title')}
        </h3>
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {developers
            .sort((a, b) => b.propertiesCount - a.propertiesCount)
            .slice(0, 6)
            .map((developer, index) => (
              <div key={developer.id} className="flex items-center space-x-4 p-4 bg-white rounded-lg shadow-sm hover:shadow-md transition-shadow">
                <div className="w-10 h-10 bg-champagne/10 rounded-full flex items-center justify-center">
                  <span className="text-champagne font-bold text-lg">
                    {index + 1}
                  </span>
                </div>
                <div className="flex-1">
                  <div className="font-semibold text-graphite">{developer.nameEn}</div>
                  <div className="text-sm text-gray-600">{developer.propertiesCount} {t('properties')}</div>
                </div>
                <div className="text-right">
                  <div className="text-sm font-semibold text-champagne">
                    {formatPrice(developer.averagePrice, developer.currency)}
                  </div>
                  <div className="text-xs text-gray-500">{t('average')}</div>
                </div>
              </div>
            ))}
        </div>
      </div>
    </div>
  )
}
